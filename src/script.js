...
const App = () => {
  ...
  return (
      <div>
        <audio id="beep" src="https://raw.githubusercontent.com/freeCodeCamp/cdn/master/build/testable-projects-fcc/audio/BeepSound.wav" type="audio/mpeg" ref={audioClip}  />
        <h1>25 + 5 Clock</h1>
        <div id="controls">
          <div>
            <h2 id="break-label">Break Length</h2>
            <div class="center-items">
            <button id="break-increment" ref={breakIncBtn} onClick={changeLength}>+</button>
            <p id="break-length">{breakLength}</p>
            <button id="break-decrement" ref={breakDecBtn} onClick={changeLength}>-</button>
            </div>
          </div>
          <div>
            <h2 id="session-label">Session Length</h2>
            <div class="center-items">
            <button id="session-increment" ref={sessionIncBtn} onClick={changeLength}>+</button>
            <p id="session-length">{sessionLength}</p>
            <button id="session-decrement" ref={sessionDecBtn} onClick={changeLength}>-</button>
            </div>
          </div>
        </div>
        <div id="timer">
          <div id="clock">
          <p id="timer-label" ref={typeText}>{type}</p>
          <p id="time-left" ref={timeText}>{`${time / 60 < 10 ? '0' + Math.floor(time / 60) : Math.floor(time / 60)}:${time % 60 < 10 ? '0' + time % 60 : time % 60}`}</p>
          </div>
          <div id="timer-btns">
            <button id="start_stop" onClick={handleClick}>{text}</button>
